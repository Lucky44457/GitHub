...
async def copy_message_with_chat_id(app, userbot, sender, chat_id, message_id, edit):
    ...
    try:
        # ✅ FIX 1: String username ko numeric ID me convert karo pehle
        if isinstance(chat_id, str) and not chat_id.lstrip('-').isdigit():
            try:
                ...
                print(f"Chat resolve failed: {resolve_err}")
                await edit.edit("❌ Channel resolve nahi hua. Bot/userbot ko channel me add karo.")
                return

        # ✅ FIX 2: Userbot se message fetch karo (public + private dono ke liye)
        if userbot:
        ...
        if not msg or msg.empty or msg.service:
            await edit.edit("❌ Message nahi mila ya empty hai.")
            return
        ...
        # ✅ FIX 3: Text message + LOG_GROUP me copy
        if msg.text:
            ...
            await result.copy(LOG_GROUP)  # ✅ LOG fix
            await edit.delete()
            return

        # ✅ FIX 4: Media message + LOG_GROUP me copy
        if msg.media:
        ...
        # Fallback: Agar upar se nahi gaya toh userbot se download karke upload karo
        if result is None:
            await edit.edit("Trying fallback method...")
            try:
                if userbot:
            ...
            if userbot:
                ...
                if isinstance(chat_id, str):
                    chat_id = (await app.get_chat(f"@{chat_id}")).id
                msg = await app.get_messages(chat_id, message_id)
