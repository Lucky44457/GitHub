...
async def cache_important_peers():
    ...
    for peer_id in peers:
        try:
            ...
            print(f"✅ Peer cached: {peer_id}", flush=True)
        except Exception as e:
            print(f"⚠️ Peer cache failed for {peer_id}: {e}", flush=True)
            print("   → Bot ko us channel/group me admin banao aur ek message bhejo, phir restart karo.")


async def restrict_bot():
    """
    ✅ Bot startup sequence — PYROGRAM PART ONLY.
    ⚠️ FIX: Telethon/userbot start yahan se HATA DIYA gaya, kyunki
    pehle ye function Telethon ka connect hone tak block karta tha —
    agar Koyeb pe Telegram DC se connection slow/hang ho jaye, toh
    command handlers (jo iske BAAD import hote the) kabhi register
    hi nahi hote the, isliye bot kisi command ka reply nahi deta tha.
    Telethon/userbots ab start_extra_clients() me background task ke
    taur pe start honge.
    """
...
async def start_extra_clients():
    """
    ✅ Telethon + userbot clients + peer cache — background task.
    Agar ye hang/fail ho jaye, tab bhi Pyrogram command handlers
    kaam karte rahenge. Har step timeout + try/except me wrapped hai.
    """
    try:
        await asyncio.wait_for(sex.start(bot_token=BOT_TOKEN), timeout=30)
        print(f"✅ Telethon client started", flush=True)
    except Exception as e:
        print(f"⚠️ Telethon start failed/timed out: {e}", flush=True)
        print("   Bot ke normal commands (Pyrogram) is se affect nahi honge.")

    try:
        await cache_important_peers()
    except Exception as e:
        print(f"⚠️ Peer cache step failed: {e}", flush=True)

    if pro:
        try:
            await asyncio.wait_for(pro.start(), timeout=30)
        except Exception as e:
            print(f"⚠️ 'pro' userbot start failed/timed out: {e}", flush=True)
    if userrbot:
        try:
            await asyncio.wait_for(userrbot.start(), timeout=30)
        except Exception as e:
            print(f"⚠️ 'userrbot' userbot start failed/timed out: {e}", flush=True)
