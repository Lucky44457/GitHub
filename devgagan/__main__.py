...
print("🚀🚀🚀 BUILD MARKER v2.3.1-FIX — agar ye line dikh rahi hai, naya code deploy hua hai 🚀🚀🚀", flush=True)

# ✅ uvloop — async event loop 30-40% faster
try:
    import uvloop
    asyncio.set_event_loop_policy(uvloop.EventLoopPolicy())
    print(f"✅ uvloop enabled", flush=True)
except ImportError:
    print(f"⚠️ uvloop not found, using default loop", flush=True)
...
from devgagan import app, restrict_bot, start_extra_clients
...
async def warm_up_peers():
    ...
    for peer_id in peers:
        try:
            ...
            print(f"✅ Peer already cached: {peer_id}", flush=True)
        except Exception:
            try:
                msg = await app.send_message(peer_id, ".")
                await msg.delete()
                print(f"✅ Peer warmed up via message: {peer_id}", flush=True)
            except Exception as e:
                print(f"⚠️ Peer warm-up failed for {peer_id}: {e}", flush=True)
...
async def devggn_boot():
    # ✅ FIX: restrict_bot() ab sirf Pyrogram start karta hai (fast, non-blocking)
    await restrict_bot()
    print(f"✅ Pyrogram bot client started", flush=True)

    # ✅ FIX: Command handlers TURANT import/register karo — Telethon ke
    #         connect hone ka wait mat karo. Isse agar Telethon Koyeb pe
    #         hang/slow ho, tab bhi saare bot commands turant kaam karenge.
    for all_module in ALL_MODULES:
        importlib.import_module("devgagan.modules." + all_module)
    print(f"✅ Command handlers registered", flush=True)

    # ✅ Telethon + userbot clients background me start honge —
    #    inka slow/fail hona command handling ko block nahi karega
