...
async def is_allowed(user_id, message):
    ...
    return False, (
        ...
        f"Or contact @{ADMIN_USERNAME} to upgrade to premium."
    )
...
# ✅ FIX: Persistent userbot — session_string MemoryStorage me peer cache nahi hota
# Ek baar start karo, alive rakho, get_dialogs() se peers preload karo
_active_userbots = {}

async def initialize_userbot(user_id):
    global _active_userbots

    # Already running client hai toh reuse karo
    if user_id in _active_userbots:
        existing = _active_userbots[user_id]
        try:
    ...
    if data and data.get("session"):
        try:
            ...
            # ✅ Peer cache: dialogs fetch karo — private channel ka access_hash milega
            # limit=200 rakha hai — Koyeb ke strict TCP timeout se bachne ke liye
            # VPS/Heroku pe koi fark nahi padta, wahan bhi same kaam karta hai
            try:
                count = 0
                ...
                print(f"Dialogs preload warning: {e}")

            _active_userbots[user_id] = userbot
            return userbot
        except Exception as e:
...
async def batch_link(_, message):
    ...
    for attempt in range(3):
        start = await app.ask(message.chat.id, "Please send the start link.\n\n<blockquote>Maximum tries 3</blockquote>", parse_mode=enums.ParseMode.HTML)
