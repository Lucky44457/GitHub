...
def thumbnail(sender):
    """User ka custom thumbnail lo, nahi hai toh None"""
    path = f'{sender}.jpg'
...
async def preload_peers(client):
    try:
        async for dialog in client.get_dialogs():
            pass
        print("✅ Peer cache loaded")
    except Exception as e:
        print(f"❌ Peer preload failed: {e}")

if STRING:
    from devgagan import pro
    print("App imported from devgagan.")

    async def start_pro_client():
        try:
            await pro.start()
            await preload_peers(pro)
            print("✅ Userbot started with peer cache")
        except Exception as e:
            print(f"❌ Userbot startup failed: {e}")
...
async def upload_media(sender, target_chat_id, file, caption, edit, topic_id):
    try:
        ...
        # ✅ FIX: User ka custom thumbnail pehle check karo
        custom_thumb = thumbnail(sender)
        if custom_thumb:
            thumb_path = custom_thumb
        else:
            # No custom thumb — video ke liye screenshot banao
            try:
...
async def resolve_channel(userbot, chat):
    try:
        if isinstance(chat, int):
            try:
                await userbot.resolve_peer(chat)
            except PeerIdInvalid:
                print("⚠️ Peer not cached. Loading dialogs...")
                await preload_peers(userbot)
                await userbot.resolve_peer(chat)
        else:
        ...
        print(f"❌ Peer resolve failed: {e}")
        return False


async def safe_get_message(userbot, chat, msg_id):
    try:
        ok = await resolve_channel(userbot, chat)

        if not ok:
            return None

        return await userbot.get_messages(chat, msg_id)

    except Exception as e:
        print(f"❌ safe_get_message error: {e}")
        return None
...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        # ✅ FIX: Peer resolve karo pehle — fresh userbot session me peer cached nahi hota
        try:
            await userbot.get_chat(chat)
        except Exception as e:
            print(f"Peer resolve warning: {e}")
        msg = await safe_get_message(userbot, chat, msg_id)
...
async def copy_message_with_chat_id(app, userbot, sender, chat_id, message_id, edit):
    ...
    try:
        # Public channel check (username based)
        is_public = isinstance(chat_id, str) and not str(chat_id).lstrip('-').isdigit()

        if is_public:
            # ✅ PUBLIC: app se fetch → msg.chat.id se instant copy (no download)
            ...
            result = await app.copy_message(target_chat_id, source_chat_id, msg.id, reply_to_message_id=topic_id)
            if result:
                try:
                    await result.copy(LOG_GROUP)
                except Exception:
        ...
        # ✅ PRIVATE: original working code (untouched)
        msg = await app.get_messages(chat_id, message_id)
        custom_caption = get_user_caption_preference(sender)
