...
async def copy_message_with_chat_id(app, userbot, sender, chat_id, message_id, edit):
    ...
    try:
        # ✅ Public: bot.get_messages(username) → copy_message(msg.chat.id) — instant
        # ✅ Private: download + upload_media — thumbnail support
        is_public = isinstance(chat_id, str) and not str(chat_id).lstrip('-').isdigit()

        # Fetch message — public: app, private: userbot
        fetch_client = userbot if (userbot and not is_public) else app
        ...
        # Use msg.chat.id (numeric, already cached after get_messages)
        source_chat_id = msg.chat.id

        # Text message — always copy
        if msg.text:
        ...
        if msg.media:
            if is_public:
                # ✅ PUBLIC: instant copy (no download) — exactly like the reference project
                result = await app.copy_message(target_chat_id, source_chat_id, msg.id, reply_to_message_id=topic_id)
                if result:
                    try:
                        await result.copy(LOG_GROUP)
                ...
                # ✅ PRIVATE: download + upload_media — thumbnail lagega
