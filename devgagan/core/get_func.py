...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        try:
            ...
            try:
                # ✅ FIX: limit=200 — Koyeb TCP timeout se bachne ke liye
                # VPS/Heroku pe koi fark nahi, teeno pe compatible
                count = 0
...
async def get_final_caption(msg, sender):
    # If media_only mode is ON, return empty caption
    if is_media_only(sender):
        return None
...
async def copy_message_with_chat_id(app, userbot, sender, chat_id, message_id, edit):
    ...
    try:
        is_public = isinstance(chat_id, str) and not str(chat_id).lstrip('-').isdigit()
        ...
        if is_public:
            # PUBLIC: instant copy via msg.chat.id
            msg = await app.get_messages(chat_id, message_id)
            if msg and not msg.empty and not msg.service:
                if is_media_only(sender) and msg.media:
                    # media_only ON: send media without caption
                    custom_caption = get_user_caption_preference(sender)
                    result = await send_media_message(app, target_chat_id, msg, "", topic_id)
                else:
        ...
        # PRIVATE: original flow
        msg = await app.get_messages(chat_id, message_id)
        custom_caption = get_user_caption_preference(sender)
        final_caption = "" if is_media_only(sender) else format_caption(msg.caption or '', sender, custom_caption)
        ...
        if result is None:
            ...
            final_caption = "" if is_media_only(sender) else format_caption(msg.caption.markdown if msg.caption else "", sender, custom_caption)
            file = await userbot.download_media(
                msg,
                progress=progress_bar,
                progress_args=("╭─────────────────────╮\n│      **__Downloading__...**\n├─────────────────────", edit, time.time())
...
def is_media_only(user_id):
    """Check if user has Media Only mode enabled"""
    user_data = collection.find_one({'user_id': user_id})
    return user_data.get('media_only', False) if user_data else False
...
async def send_settings_message(chat_id, user_id):
    ...
    buttons = [
        ...
        [Button.inline("Media Only Mode", b'mediaonly')],
        [Button.url("Report Errors", "https://t.me/DARKEND_X")]
    ]
...
async def callback_query_handler(event):
    ...
    elif event.data == b'mediaonly':
        user_data = collection.find_one({'user_id': user_id})
        current = user_data.get('media_only', False) if user_data else False
        new_val = not current
        collection.update_one(
            {'user_id': user_id},
            {'$set': {'media_only': new_val}},
            upsert=True
        )
        status = "ON ✅" if new_val else "OFF ❌"
        await event.edit(
            f"**Media Only Mode: {status}**\n\n"
            "When ON:\n"
            "• Only video/PDF will be sent\n"
            "• Caption will be removed\n\n"
            "When OFF:\n"
            "• Caption will be included as set in your settings"
        )
