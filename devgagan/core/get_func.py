...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    try:
        ...
        # ✅ FIX: Target peer cache karo warna "Peer id invalid" error aata hai
        try:
            await app.get_chat(int(target_chat_id))
        except Exception as e:
            print(f"Target peer cache warning: {e}")
...
# ✅ FIX: LOG_GROUP me bhi bhejo (was missing before)
async def clone_message(app, msg, target_chat_id, topic_id, edit_id, log_group):
    try:
        edit = await app.edit_message_text(target_chat_id, edit_id, "Cloning...")
        sent = await app.send_message(target_chat_id, msg.text.markdown, reply_to_message_id=topic_id)
        await sent.copy(log_group)  # ✅ FIXED: LOG_GROUP me copy
        await edit.delete()
    except Exception as e:
        print(f"clone_message error: {e}")
...
async def clone_text_message(app, msg, target_chat_id, topic_id, edit_id, log_group):
    try:
        ...
        print(f"clone_text_message error: {e}")
...
# ✅ FIX: Public channel support + LOG_GROUP fix
# OLD: app.get_messages() → bot se fetch (public channels me fail)
# NEW: userbot.get_messages() → userbot se fetch (sab jagah kaam karta hai)
async def copy_message_with_chat_id(app, userbot, sender, chat_id, message_id, edit):
    ...
    try:
        # ── Step 1: Source chat resolve (username → numeric ID) ──
        if isinstance(chat_id, str) and not chat_id.lstrip('-').isdigit():
            try:
                chat_obj = await (userbot or app).get_chat(chat_id)
                chat_id = chat_obj.id
            except Exception as e:
                await edit.edit(f"❌ Source channel resolve failed: {e}")
                return

        # ── Step 2: Target chat resolve (Peer ID error fix) ──
        topic_id = None
        if '/' in str(target_chat_id):
            target_chat_id, topic_id = map(int, str(target_chat_id).split('/', 1))
        try:
            target_chat_id = int(target_chat_id)
            await app.get_chat(target_chat_id)  # target peer cache karo
        except Exception as e:
            print(f"Target peer cache warning: {e}")

        # ── Step 3: Message fetch (userbot preferred) ──
        fetch_client = userbot if userbot else app
        msg = await fetch_client.get_messages(chat_id, message_id)
        ...
        # ── Step 5: Media — download via fetch_client, upload via bot ──
        # NOTE: file_id shortcut (send_media_message) REMOVED intentionally.
        # Userbot ka file_id bot use nahi kar sakta → MEDIA_EMPTY error aata tha.
        # Correct flow: download → upload (dono ke liye kaam karta hai).
        if msg.media:
            final_caption = format_caption(
                msg.caption.markdown if msg.caption else '', sender, custom_caption
            )
            file_size = get_message_file_size(msg)
            ...
            if file_size > size_limit:
                ...
                file = await fetch_client.download_media(msg)
                file = await rename_file(file, sender)
                if freecheck == 1 or pro is None:
                    await edit.delete()
            ...
            # Normal size: download then upload
            await edit.edit("**Downloading...**")
            file = await fetch_client.download_media(
                msg,
                progress=progress_bar,
                progress_args=(
                    "╭─────────────────────╮\n│      **__Downloading__...**\n├─────────────────────",
        ...
        print(f"copy_message_with_chat_id error: {e}")
    finally:
        if file and os.path.exists(file):
            os.remove(file)
