...
async def batch_link(_, message):
    ...
    for attempt in range(3):
        await app.send_message(message.chat.id, "Please send the start link.\n\n> Maximum tries 3")
        start = await app.listen(message.chat.id)
    ...
    for attempt in range(3):
        await app.send_message(message.chat.id, f"How many messages do you want to process?\n\n> Max limit {max_batch_size}")
        num_messages = await app.listen(message.chat.id)
        try:
